...
def test_label_split_data(tmp_path):
    ...
    def shuffle(x):
        # place "c.pdf" devant si présent (mimique du test d'origine)
        mypdf = None
        for c in x:
    ...
    def copy_file(file_path: Path, to_file_path: Path) -> None:
        rel = str(to_file_path).replace(str(BASE_PATH), "").replace("\\", "/")
    ...
    result = data_split.label_split_data(label_split_data_input)

    expected = [
        '/output_integration/c.pdf',
    ...
    for p in files_copied:
        assert p in expected

    data_manager_mock.create_directory.assert_called()
    assert result.number_file_train_by_label == 1
    assert result.number_file_test_by_label == 1
    assert result.number_file_evaluate_by_label == 1
    assert result.number_labeled_data == 9
